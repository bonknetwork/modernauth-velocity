                ...
                if (responseCode == 200) {
                    ...
                    proxyServer.getScheduler().buildTask(plugin, () -> {
                        if (exists) {
                            ...
                            Optional<AccountData> accountOpt = nLoginAPI.getApi().getAccount(Identity.ofKnownName(player.getUsername()));
    ...
    public void startAuthentication(Player player, boolean changePassword) {
        ...
        proxyServer.getScheduler().buildTask(plugin, () -> {
            ...
            proxyServer.getScheduler().buildTask(plugin, () -> {
                ...
                player.sendMessage(loginMsg + " " + authUrl);
                ScheduledTask task = proxyServer.getScheduler().buildTask(plugin, new AuthTask(plugin, this, player, token, changePassword))
